...
class SPGG_Qlearning(nn.Module):
    ...
    def run(self, num):
        ...
        d_matrix, c_matrix = self.type_matrix_to_three_matrix(type_t_matrix)
        D_Y, C_Y, D_Value, C_Value, all_value, count_0, count_1, CC, DD, CD, DC = self.cal_fra_and_value(
            D_Y, C_Y, D_Value, C_Value, all_value, type_t_matrix, type_t_matrix,  # 注意：两个参数相同，表示没有更新
        ...
        for i in tqdm(range(self.epoches)):
            ...
            # 记录数据（这里记录的是当前步的数据，即更新前的状态？需要确认逻辑）
            # 根据 Fermi 的逻辑，应该记录更新后的状态
            d_matrix, c_matrix = self.type_matrix_to_three_matrix(type_t1_matrix)  # 改为记录更新后的
